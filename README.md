...
[^1]: The original paper uses two parameters _σ_ and _τ_, splitting the PRF input as (_x_,_y_) with _x_ ∈ [_σ_] and _y_ ∈ [τ] to guarantee that at least the _y_-half could always be extracted from the ciphertext. This version eliminates that constraint by allowing full counter recovery via brute-force search, making _y_ and hence _τ_ redundant: security depends only on _σ_ (search difficulty), not on the PRF input's internal structure.
